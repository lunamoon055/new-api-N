...
.PHONY: all build-frontend build-frontend-classic build-all-frontends start-backend dev dev-api dev-api-rebuild dev-web dev-web-classic prod-env prod-deploy prod-ps prod-logs prod-restart reset-setup
...
prod-env:
	@./scripts/create-production-env.sh "$${PUBLIC_URL:-}"

prod-deploy:
	@./scripts/deploy-remote.sh

prod-ps:
	@./scripts/remote-manage.sh ps

prod-logs:
	@./scripts/remote-manage.sh logs "$${SERVICE:-new-api}"

prod-restart:
	@./scripts/remote-manage.sh restart "$${SERVICE:-new-api}"
