...
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	...
	// 异步图片可使用公开别名：先解析本次渠道映射，再按实际上游模型校验能力。
	// 每次重试 InitChannelMeta 都重置快照，不能复用上一个渠道的映射结果。
	imageModelMapped := strings.HasPrefix(info.RequestURLPath, "/v1/images/async-generations") &&
		common.IsBaseURLHost(info.ChannelBaseUrl, common.LinkskyProviderHost)
	if imageModelMapped {
	...
	if resp != nil && !isTaskSubmitSuccessStatus(resp.StatusCode) {
	...
	if info.PriceData.VideoBillingMode != billing_setting.VideoBillingModeFixed {
		if adjustedRatios := adaptor.AdjustBillingOnSubmit(info, taskData); len(adjustedRatios) > 0 {
			// 基于调整后的 ratios 重新计算 quota
			finalQuota = recalcQuotaFromRatios(info, adjustedRatios)
			info.PriceData.OtherRatios = adjustedRatios
			info.PriceData.Quota = finalQuota
			info.PriceData.AppliedOtherRatios = true
		}
	}
...
func isTaskSubmitSuccessStatus(statusCode int) bool {
