...
func OpenaiRealtimeHandler(c *gin.Context, info *relaycommon.RelayInfo) (*types.NewAPIError, *dto.RealtimeUsage) {
	...
	usageAccumulator := &realtimeUsageAccumulator{}
	...
	gopool.Go(func() {
		...
		for {
			...
			default:
				...
				usageAccumulator.addInput(textToken, audioToken)
		...
		for {
			...
			default:
				...
				if realtimeEvent.Type == dto.RealtimeEventTypeResponseDone {
					...
					var cumulativeUsage *dto.RealtimeUsage
					if realtimeUsage != nil {
						cumulativeUsage = usageAccumulator.complete(realtimeUsage, 0, 0)
					} else {
						realtimeStateMu.Lock()
						textToken, audioToken, err := service.CountTokenRealtime(info, *realtimeEvent, info.UpstreamModelName)
						if err != nil {
						...
						realtimeStateMu.Unlock()
						cumulativeUsage = usageAccumulator.complete(nil, textToken, audioToken)
					}
					if err := service.ReserveWssQuota(c, info, cumulativeUsage); err != nil {
						errChan <- fmt.Errorf("error reserving realtime usage: %v", err)
						return
					}
					logger.LogInfo(c, fmt.Sprintf("realtime streaming cumulative usage: %v", cumulativeUsage))
					...
					logger.LogInfo(c, fmt.Sprintf("type: %s, textToken: %d, audioToken: %d", realtimeEvent.Type, textToken, audioToken))
					usageAccumulator.addOutput(textToken, audioToken)
				}
	...
	// Stop the other reader and wait until no goroutine can mutate usage.
	_ = clientConn.Close()
	_ = targetConn.Close()
	readers.Wait()

	return nil, usageAccumulator.finalize()
}
...
func OpenaiHandlerWithUsage(c *gin.Context, info *relaycommon.RelayInfo, resp *http.Response) (*dto.Usage, *types.NewAPIError) {
	...
	responseBody = service.RewriteImageGenerationResponse(c, responseBody)
