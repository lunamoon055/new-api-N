...
func EpayNotify(c *gin.Context) {
	...
	if !isEpayWebhookEnabled() {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("易支付 webhook 被拒绝 reason=webhook_disabled path=%q client_ip=%s", path, c.ClientIP()))
		c.Status(http.StatusForbidden)
		_, _ = c.Writer.Write([]byte("fail"))
		return
	}

	params, err := paymentCallbackParams(c)
	if err != nil {
		status := paymentWebhookReadErrorStatus(err)
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("易支付 webhook 参数解析失败 path=%q client_ip=%s status=%d error=%q", path, c.ClientIP(), status, err.Error()))
		c.Status(status)
		_, _ = c.Writer.Write([]byte("fail"))
		return
	}
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 webhook 收到请求 path=%q client_ip=%s method=%s param_count=%d", path, c.ClientIP(), c.Request.Method, len(params)))

	if len(params) == 0 {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("易支付 webhook 参数为空 path=%q client_ip=%s", path, c.ClientIP()))
	...
	client := GetEpayClient()
	if client == nil {
		logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 client 未初始化 path=%q client_ip=%s", path, c.ClientIP()))
	...
	if err != nil || !verifyInfo.VerifyStatus {
		if err != nil {
			logger.LogWarn(c.Request.Context(), fmt.Sprintf("易支付 webhook 验签失败 path=%q client_ip=%s verify_error=%q", path, c.ClientIP(), err.Error()))
		} else {
			logger.LogWarn(c.Request.Context(), fmt.Sprintf("易支付 webhook 验签失败 path=%q client_ip=%s verify_status=false", path, c.ClientIP()))
		}
		c.Status(http.StatusBadRequest)
		_, _ = c.Writer.Write([]byte("fail"))
		return
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 充值处理成功 trade_no=%s callback_type=%s client_ip=%s", verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP()))
	_, _ = c.Writer.Write([]byte("success"))
}
