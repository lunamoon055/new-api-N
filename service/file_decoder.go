...
func GetFileTypeFromUrl(c *gin.Context, url string, reason ...string) (string, error) {
	...
	if err != nil {
		common.SysLog(fmt.Sprintf("fail to get file type from url: %s, error: %s", common.MaskSensitiveInfo(url), err.Error()))
		return "", err
	}
	defer response.Body.Close()

	if response.StatusCode != 200 {
		logger.LogError(c, fmt.Sprintf("failed to download file from %s, status code: %d", common.MaskSensitiveInfo(url), response.StatusCode))
